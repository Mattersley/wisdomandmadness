...
const About = () => {
  return (
    ...
    >
      <h2
        className={`${vagraFont.className} bg-linear-to-tr from-[#3F5EFB] to-[#FC466B] bg-clip-text text-[7rem] text-transparent`}
      >
        About
      </h2>
      <motion.p
        className="mt-4 mb-12 cursor-pointer rounded-full border border-indigo-500/20 px-6 py-1 font-mono text-xs tracking-[0.25em] text-indigo-500/60 uppercase transition-all duration-300 hover:border-indigo-500 hover:bg-indigo-500 hover:text-white"
        whileHover={{ scale: 1.05 }}
        whileTap={{ scale: 0.98 }}
      >
        CORE TEAM
      </motion.p>
      <div className="mb-10 flex flex-col items-center text-center md:grid md:w-240 md:grid-cols-2 md:flex-row md:items-start md:gap-5 md:text-right lg:w-[90%]">
        <ProfileCard />
