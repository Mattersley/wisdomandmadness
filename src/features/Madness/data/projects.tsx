import Pando from '@/features/Madness/Portfolio/components/CaseStudies/Pando/Pando'
import Cupendium from '@/features/Madness/Portfolio/components/CaseStudies/Cupendium/Cupendium'
import { ProjectType } from '@/features/Madness/data/projects.types'

export const projects: ProjectType[] = [
  {
    id: 1,
    name: 'Pando',
    tagline:
      'Gourmet donut brand package, loyalty system/monthly subscription web app & bakery management software',
    overview:
      'Pandō Donuts Inc. is a gourmet donut brand that offers a variety of creative and inspired donuts. The package provided included logos, a mascot, packaging, a menu website, a web app, bakery management software, UI/UX, and marketing materials. The loyalty system and monthly subscription web app allow customers to earn rewards and discounts for their purchases, and also order special edition boxes with seasonal and themed flavours. The bakery management software allows management of inventory, orders, operations, and customer data.',
    z: 20,
    bgColours: { 1: '#FFE6FA', 2: '#E3FDF5' },
    bgOpacity: 1,
    colourPalettes: {
      pal: ['#800080', '#CC99CC', '#333333'],
      bg: ['#FFE6FA', '#E3FDF5']
    },
    dark: false,
    image: {
      src: '/images/Portfolio/CaseStudies/PandoLogo.png',
      alt: 'Pando Logo',
      width: 150,
      height: 75
    },
    served: [
      'Branding',
      'Packaging',
      'Website',
      'Web App',
      'UI/UX',
      'Marketing'
    ],
    stack: {
      NextJs: 'https://www.nextjs.org',
      React: 'https://www.reactjs.org',
      StoryBook: 'https://storybook.js.org/',
      Supabase: 'https://www.supabase.io',
      'Three.js': 'https://www.threejs.org',
      Typescript: 'https://www.typescriptlang.org',
      WordPress: 'https://www.wordpress.org'
    },
    urls: [
      {
        url: 'https://www.pandonuts.club',
        favi: '/images/Portfolio/Favis/PandoClub.png'
      },
      {
        url: 'https://www.pandonuts.com',
        favi: '/images/Portfolio/Favis/Pandonuts.png'
      }
    ],
    theme: 'light',
    infoPosition: 'center',
    content: <Pando />
  },
  {
    id: 2,
    name: 'Cupendium',
    z: 60,
    bgColours: { 1: '#f4d03f', 2: '#16a085' },
    colourPalettes: { bg: ['#f4d03f', '#16a085'] },
    bgOpacity: 0.2,
    dark: false,
    image: {
      src: '/images/Portfolio/CaseStudies/CupendiumLogo.png',
      alt: 'Cupendium Logo',
      width: 400,
      height: 200
    },
    served: ['Branding', 'Web App', 'UI/UX', 'Concept'],
    urls: [
      {
        url: 'https://www.cupendium.com',
        favi: '/images/Portfolio/Favis/CupendiumICO.svg'
      }
    ],
    tagline: 'Tasting Log, Digital & Printable Tools for Pour-Over Coffee',
    overview:
      'Cupendium is a digital brewing system for pour-over coffee. The app allows coffee enthusiasts to log brewing parameters, beans, and other information related to their brews, then find the best possible brewing method for their needs. The optimization system will then recommend parameters for their next brew dependent upon their tasting notes.',
    stack: {
      Motion: 'https://motion.dev/',
      NextJs: 'https://www.nextjs.org',
      React: 'https://www.reactjs.org',
      Supabase: 'https://www.supabase.io',
      Stripe: 'https://www.stripe.com',
      Typescript: 'https://www.typescriptlang.org'
    },
    theme: 'light',
    content: <Cupendium />
  },
  {
    id: 3,
    name: 'Druid',
    z: 60,
    bgColours: { 1: '#363C48', 2: '#1B1B19' },
    bgImage: '/images/Portfolio/CaseStudies/DruidBackground.jpg',
    bgOpacity: 1,
    dark: true,
    image: {
      src: '/images/Portfolio/CaseStudies/DruidLogoWhite.png',
      alt: 'Druid Logo',
      width: 130,
      height: 100
    },
    served: ['Branding', 'Concept', 'Menus'],
    urls: [],
    tagline: 'Confectionary. Coffee. Cocktails. Carpe Noctem.',
    overview:
      'A multi faceted new concept in hospitality. Building upon extensive knowledge in distilling, chemistry, cocktails, foraging, farming, coffee, tea, and high concept pastry. Crafting expansive new experiences and products with a focus on pre-Roman and ancient rites and rituals.',
    stack: {},
    theme: 'dark'
  },
  {
    id: 4,
    name: 'Naturalist',
    z: 20,
    bgColours: { 1: '#d6e9d4', 2: '#ffffff' },
    bgOpacity: 1,
    dark: false,
    image: {
      src: '/images/Portfolio/CaseStudies/NaturalistLogo2.png',
      alt: 'The Naturalist Logo',
      width: 450,
      height: 200
    },
    served: ['Branding', 'Packaging'],
    urls: [],
    tagline:
      'Brand identity and design systems for global environmental scale.',
    overview:
      'A complete structural overhaul of Deep Forest’s global digital platform. We designed an open-source component kit that enabled their internal product engineering teams to deploy 4x faster.',
    stack: {},
    theme: 'light',
    infoPosition: 'right'
  },
  {
    id: 5,
    name: 'Hattersleys',
    z: 20,
    bgColours: { 1: '#f4d03f', 2: '#16a085' },
    bgImage: '/images/Portfolio/CaseStudies/HattersleysBackground.png',
    bgOpacity: 1,
    dark: false,
    image: {
      src: '/images/Portfolio/CaseStudies/HattersleysLogo.png',
      alt: 'Hattersleys Logo',
      width: 250,
      height: 200
    },
    served: ['Branding', 'Packaging', 'Website', 'Marketing'],
    urls: [],
    tagline: 'Tasting Log, Digital & Printable Tools for Pour-Over Coffee',
    overview:
      'Serene Lake required a high-end visual storytelling engine that could handle heavy WebGL assets without dipping below 60fps. Our design focused on fluid interactive micro-moments.',
    stack: {},
    theme: 'light',
    infoPosition: 'center'
  },
  {
    id: 6,
    name: 'SpiritFish',
    z: 20,
    bgColours: { 1: '#9cb49c', 2: '#9cb49c' },
    bgImage: '/images/Portfolio/CaseStudies/SpiritfishBackground.jpg',
    bgOpacity: 1,
    dark: true,
    image: {
      src: '/images/Portfolio/CaseStudies/SpiritfishLogo1.png',
      alt: 'Spiritfish Logo',
      width: 450,
      height: 100
    },
    served: ['Branding', 'Web App'],
    urls: [],
    tagline: 'Tasting Log, Digital & Printable Tools for Pour-Over Coffee',
    overview:
      'Serene Lake required a high-end visual storytelling engine that could handle heavy WebGL assets without dipping below 60fps. Our design focused on fluid interactive micro-moments.',
    stack: {},
    theme: 'dark'
  },
  {
    id: 7,
    name: 'GlassRoots',
    z: 40,
    bgColours: { 1: '#ffffff', 2: '#27aae1' },
    bgOpacity: 1,
    dark: false,
    image: {
      src: '/images/Portfolio/CaseStudies/GlassrootsLogo.png',
      alt: 'Glassroots Logo',
      width: 250,
      height: 200
    },
    served: ['Branding', 'Concept'],
    urls: [],
    tagline: 'Tasting Log, Digital & Printable Tools for Pour-Over Coffee',
    overview:
      'Serene Lake required a high-end visual storytelling engine that could handle heavy WebGL assets without dipping below 60fps. Our design focused on fluid interactive micro-moments.',
    stack: {},
    theme: 'light'
  },
  {
    id: 8,
    name: 'LI',
    z: 40,
    bgColours: { 1: '#393939', 2: '#393939' },
    bgImage: '/images/Portfolio/CaseStudies/LIBackground.png',
    bgOpacity: 1,
    dark: false,
    image: {
      src: '/images/Portfolio/CaseStudies/LI.png',
      alt: 'LI logo',
      width: 350,
      height: 200
    },
    served: ['Brand ID', 'Logo Design', 'Typography'],
    urls: [],
    tagline: 'Tasting Log, Digital & Printable Tools for Pour-Over Coffee',
    overview:
      'Serene Lake required a high-end visual storytelling engine that could handle heavy WebGL assets without dipping below 60fps. Our design focused on fluid interactive micro-moments.',
    stack: {},
    theme: 'light'
  },
  {
    id: 9,
    name: 'CIMS',
    z: 40,
    bgColours: { 1: '#303b74', 2: '#303b74' },
    bgImage: '/images/Portfolio/CaseStudies/CIMSBackground.png',
    bgOpacity: 1,
    dark: false,
    image: {
      src: '/images/Portfolio/CaseStudies/CIMSLogo1.png',
      alt: 'cims logo',
      width: 250,
      height: 400
    },
    served: ['Brand ID', 'Logo Design'],
    urls: [],
    tagline: 'Tasting Log, Digital & Printable Tools for Pour-Over Coffee',
    overview:
      'Serene Lake required a high-end visual storytelling engine that could handle heavy WebGL assets without dipping below 60fps. Our design focused on fluid interactive micro-moments.',
    stack: {},
    theme: 'light',
    infoPosition: 'center'
  }
]
